...
public class JpaInfrastructureConfig {
    ...
    public JpaInfrastructureConfig(ApplicationEventPublisher applicationEventPublisher) {
        this.applicationEventPublisher = applicationEventPublisher;
    }

    @Bean
    public TicketRepository ticketRepository(TicketJpaRepository ticketJpaRepository) {
        return new JpaTicketRepository(applicationEventPublisher, ticketJpaRepository);
    }
    ...
    public WatcherRepository watcherRepository(WatcherJpaRepository watcherJpaRepository, WatchJpaRepository watchJpaRepository) {
        return new JpaWatcherRepository(applicationEventPublisher, watcherJpaRepository, watchJpaRepository);
    }

    @Bean
    public WatchRepository watchRepository(WatchJpaRepository watchJpaRepository) {
        return new JpaWatchRepository(applicationEventPublisher, watchJpaRepository);
    }
}
